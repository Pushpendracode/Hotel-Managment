    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs font-medium text-gray-500 mb-1 block">Resident *</label>
            <select value={residentId} onChange={e => setResidentId(e.target.value)}
              ...
              {residents.filter(r => r.status === 'active').map(r => (
                <option key={r._id} value={r._id}>{r.name} — Room {r.roomId?.number || '?'}</option>
              ))}
            </select>
          </div>
                ...
                <div key={i} className="flex gap-2 items-center">
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={item.amount}
                      ...
                      className="w-24 pl-5 pr-2 py-2 rounded-lg border border-gray-200 text-sm
                                 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                  {lineItems.length > 1 && (
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-xl">
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-4">
          ...
          <div className="text-2xl font-semibold text-gray-900">₹{invoice.total.toLocaleString()}</div>
