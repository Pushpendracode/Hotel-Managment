...
router.get('/seed-all', async (req, res) => {
  try {
    ...
    // Residents — including one for the test resident account
    ...
    await conn.close()
    res.json({ message:'✅ All data seeded! resident@hostelpro.com now has a resident profile linked.' })
  } catch (err) {
    res.status(500).json({ message: err.message })
  }
})
...
router.get('/seed-test-resident', async (req, res) => {
  try {
    ...
    if (!room) return res.status(400).json({ message: 'No vacant rooms available' })
